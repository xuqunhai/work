...
export default class ComponentTree {
        ...
        if (node.cid === cid) {
          ...
          if (node.children.length > 0) {
            ...
            for (var j = 0; j < node.children.length; j++) {
              stack.unshift(node.children[j]); // 深度优先遍历
            }
          }
        }
      }
  ...
  removeComponent(cid) {
    let stack = [],
      parent = this.treeData,
      index = 0;
    for (let i = 0, length = parent.children.length; index < length; index += 1) {
      let currentParent = parent;
      let child = parent.children[i];
      stack.push(child);
      while (stack.length > 0) {
    ...
    function recurse(data, c) {
      ...
      if (data.cid === cid) {
        ...
        if (data.children.length > 0) {
          data.children.map((node) => recurse(node, c));
        }
      }
    }
  ...
  copy(cid) {
    ...
    function recurse(data, c) {
      data.cid = ++uuid + '-' + uuid();
      if (data.children.length > 0) {
        data.children.map((item) => recurse(item, c));
      }
    }
    recurse(node);
    return node;
